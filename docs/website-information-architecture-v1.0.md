# 《Website Information Architecture V1.0》
...
说明：`relations/` 与各类 `topics/` 是 IA 预留，不进入 Website MVP V1.0 开发范围。产品与培训在 MVP 中只承担已批准信息的目录入口，不引入交易、账号、报名或学习系统。
...
| `/standards`（当前平台治理标准） | `/standards/`（目标 GT Package） | 存在语义冲突，等待 Chief Architect 决策 |
| `/standards/{slug}` | 待决定内部治理标准公开位置 | 不迁移 |
...
Candidate、draft、in_review、未批准 Evidence、内部平台标准、后台数据与 archived 正文默认不进入公开搜索索引。Products 与 Training 作为独立站点页面索引，可在“全站”范围出现，但不冒充 Foundation Knowledge Object。
...
## 10. Architecture Review Issues

### IA-019-01：`/standards` 语义冲突

当前网站 `/standards` 发布平台治理标准；No.019 目标架构需要 Standards 承载 GT Package。建议把公共 Standards 定义为 GT Package，而平台内部工程/生产标准迁入非公共治理入口或 About 下的独立位置。此建议涉及平台标准展示边界，必须由 Chief Architect 批准，本次不执行。
...
## 12. No.019 Scope Validation

本次提交只新增本 Architecture Document：
