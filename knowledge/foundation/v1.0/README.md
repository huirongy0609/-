...
└── jd/
    ...
    └── jd009-what-is-open-budget.md
```

- `standards/`：仅接收三份已经核实并获得批准的 V1.0 标准原件。
- `jd/`：仅接收已经核实并获得批准的JD正式原件。
- `manifest.md`：逐项记录来源、版本、批准证据、生命周期状态和建议处理。
- `../drafts/`：Foundation候选稿隔离区，不属于V1.0正式知识包，不得作为正式知识对象引用。

## 2. Foundation Knowledge Pack 定位

Foundation Knowledge Pack 计划作为后续 JD、GT、案例和标准建设的基础权威来源。当前已正式入库JD001—JD006及首个后续新增对象JD009，初始11份原件仍未齐全，因此本目录处于部分入库状态，**不得宣布为完整的唯一权威来源，不得作为缺失对象或Draft候选的替代品**。
...
- 在未经项目总架构师批准时，将任何提交审核稿作为正式知识对象入库。
